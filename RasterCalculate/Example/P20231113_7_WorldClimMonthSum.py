...
if __name__ == '__main__':
    ...
    output_folder = r'E:\Glacier_DEM_Register\Tanggula_FourYear_Data\Test_Final_20231018\1_Cartography\3_Analysis\3_Analysis_Data\20231113_2_WorldClimProcessResult\20231113_1_WCPR_MonthSum'
    ...
    for type_index, type_item in enumerate(type_list):
        for year_index, year_item in enumerate(year_list):
            ...
            temp_filter_raster_tuple = list(filter(lambda x: str(type_item) in x[1] and str(year_item) in x[1], temp_raster_tuple))
            temp_result_list = list(list(zip(*temp_filter_raster_tuple))[1])
            RC.RasterAdd(temp_result_list[0], output_folder, *temp_result_list[1:], _output_name=f'{type_item}_{year_item}')
