...
if __name__ == '__main__':
    ...
    # 循环年份
    # 循环某一年的bin
    xgboost_output_folder = r'E:\Glacier_DEM_Register\Tanggula_FourYear_Data\Test_Final_20231018\0_BaseData\2_PredictData\2_Intra\1_XGBoostData'
                ...
                for bin_index, bin_item in enumerate([i * 50 for i in range(1, 5)]):
                    ...
                    if dem_type == 'NASA':
                        ...
                        for point_files_index, point_files_item in enumerate(nasa_point_files_list):
                            ...
                            if dem_type in point_files_item and f'Bin_{bin_item}' in point_files_item and f'Month{season_item}' in point_files_item:
                                print(f'当前执行条件为:{dem_type} {year_item} {bin_item} Month{season_item}'
                                      f'已找到文件名为:{point_files_item}')
                                point_path = nasa_point_path_list[point_files_index]
                                break
                            else:
                                print(
                                    f'ERROR: 不存在符合条件为:{dem_type} {bin_item} Month{season_item}的Point Shape文件.*')
                    elif dem_type == 'SRTM':
                        print('正在执行SRTM部分...')
                        # 找到对应的point_path
                        for point_files_index, point_files_item in enumerate(srtm_point_files_list):
                            if dem_type in point_files_item in point_files_item and f'Bin_{bin_item}' in point_files_item and f'Month{season_item}' in point_files_item:
                                print(f'当前执行条件为:{dem_type} {year_item} {bin_item} Month{season_item}'
                                      f'已找到文件名为:{point_files_item}')
                    ...
                    # 生成输出预测结果的路径
                    xgboost_output_path = os.path.join(xgboost_output_folder,
                                                       f'{dem_type}_{bin_level}_Month{season_item}')
