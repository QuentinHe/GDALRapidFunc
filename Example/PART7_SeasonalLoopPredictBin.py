        ...
        for year_index, year_item in enumerate([2019]):
            ...
            for season_index, season_item in enumerate(season_list):
                ...
                for bin_index, bin_item in enumerate([i * 50 for i in range(1, 5)]):
                    ...
                    if dem_type == 'NASA':
                        ...
                        for point_files_index, point_files_item in enumerate(nasa_point_files_list):
                            ...
                            if dem_type in point_files_item and str(
                                ...
                                print(f'当前执行条件为:{dem_type} {year_item} {bin_item} Season{season_item}'
                                      f'已找到文件名为:{point_files_item}')
                                point_path = nasa_point_path_list[point_files_index]
                                break
                            else:
                                print(f'ERROR: 不存在符合条件为:{dem_type} {year_item} {bin_item} Season{season_item}的Point Shape文件.*')
                    elif dem_type == 'SRTM':
                        print('正在执行SRTM部分...')
                        # 找到对应的point_path
                    ...
                    # 生成输出预测结果的路径
                    xgboost_output_path = os.path.join(xgboost_output_folder, f'{dem_type}_{year_item}_{bin_level}_Season{season_item}')
